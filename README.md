...
1. **Library panel** — three tabs over the reusable definitions the world is
   ...
   - **tiles** — create/edit tile types (name, ascii symbol, color,
     ...
     wrap-around shift), and a selectable resolution (4×4 up to 1024×1024, art is
     rescaled on change). Persisted to localStorage; older top/sides/bottom
     art is migrated automatically. The 26 shipped tiles (terrain, water, ice,
     snow, stonework, walls, planks, thatch, farmland, lava, ash, hedge,
     scorched stone, charred tree) come with 32×32 art on every face,
     generated by the painters in `src/world/tiles/art` —
   ...
   - **characters** — creatures that carry things and render as sprites: every
     ...
     layered underneath. The library ships **the lanternbearer**, a female
     dwarf drawn at 128×128 across all five rotations with a six-frame idle and
     an eight-frame walk — she is what you and every other player are drawn as
     in the 2.5D view. Her frames are generated by the painters in
     `src/creatures/art/dwarf` rather than stored, so the saved creature file
     names the art instead of carrying a megabyte of pixels
     (`npm run dwarf:preview` writes `docs/dwarf-portrait.png`).
