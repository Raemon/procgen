...
export function VoxelLayerCanvas({
  ...
  useEffect(() => {
    if (canvas.current) redraw(canvas.current, prefab, layer, tileAssets);
  });
...
function redraw(
  ...
  paintLayer(ctx, prefab, layer - 1, tileAssets, scale, 0.28);
  paintLayer(ctx, prefab, layer, tileAssets, scale, 1);
