...
export class PrefabPreviewScene {
  ...
  showPrefab(prefab: Prefab, tileAssets: ReadOnlyTileAssets): void {
    this.clearVoxels();
    this.voxelGroup.add(...voxelMeshes(prefab, tileAssets));
...
function voxelMeshes(prefab: Prefab, tileAssets: ReadOnlyTileAssets): THREE.Mesh[] {
  ...
  for (let layer = 0; layer < prefab.layers; layer++) {
    for (let y = 0; y < prefab.depth; y++) {
      for (let x = 0; x < prefab.width; x++) {
        collectVoxel(byColor, prefab, tileAssets, x, y, layer);
      }
    }
  }
