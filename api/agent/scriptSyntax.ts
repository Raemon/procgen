...
import { listOf } from '../../abilities/abilityParams';
import type { AbilityMode } from '../../abilities/ability';
...
export function parseScriptForMode(mode: AbilityMode, body: string): ScriptParse {
...
function lineFault(mode: AbilityMode, line: ScriptLine): string | null {
  const spec = abilityFor(mode, line.action);
  if (!spec) {
