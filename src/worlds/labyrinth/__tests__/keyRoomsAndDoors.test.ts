...
import { gateLook } from '@/features/game/fixtures/fixtureAppearance';
...
import { allPuzzleKinds } from '../generate/kinds/puzzleKind';
import { RoomCells } from '../generate/kinds/roomCells';
import { PuzzleWorld } from '../rules/puzzleWorld';
import { everyGateOf, sideOfGate, type PuzzleRoomLayout } from '../generate/rooms/puzzleRoomLayout';
...
function labyrinthPuzzles(): PuzzleWorld {
  ...
  const labyrinth = store.nodes().find((node) => node.type === LABYRINTH_NODE_TYPE)!;
  return new PuzzleWorld(store, labyrinth.id, () => true, undefined, new ItemAssets(defaultItems()));
}
