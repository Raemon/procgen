...
export function fogDistancesFromCamera(cameraDistanceToPlayer: number): FogDistances {
  return {
    haze: cameraDistanceToPlayer + CHARACTER_HAZE_START_TILES,
    opaque: cameraDistanceToPlayer + CHARACTER_SIGHT_RADIUS_TILES,
  };
}

export function groundRadiusToStreamTiles(cameraDistanceToPlayer: number): number {
  return cameraDistanceToPlayer + CHARACTER_SIGHT_RADIUS_TILES;
}
