...
import { apiExcerptOf, apiLineOf, buildApiSourceIndex, resolveApiModuleFile } from './apiSourceIndex';
...
let defaultTypeCatalog: ApiTypeEntry[] | null = null;

export function buildApiTypeCatalog(root: string = process.cwd()): ApiTypeEntry[] {
  if (root === process.cwd() && defaultTypeCatalog) return defaultTypeCatalog;
  const index = buildApiSourceIndex(root);
  const catalog = orderByApiIntroduction(typesByFile(index), index, buildApiEndpointCatalog(root));
  if (root === process.cwd()) defaultTypeCatalog = catalog;
  return catalog;
}
