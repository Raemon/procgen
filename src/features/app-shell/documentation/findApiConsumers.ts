...
export function findApiConsumerCandidates(index: ApiSourceIndex): ApiConsumerCandidate[] {
  ...
  for (const file of index.files.values()) {
    if (!consumerFileIsExcluded(file.path)) collectCandidates(file, index, candidates);
  }
  return candidates;
}
...
function pathValue(
  ...
  if (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) {
    return node.text.startsWith('/api/') ? node.text : null;
  }
...
function apiPathInTemplate(node: ts.TemplateExpression): string | null {
  ...
  return apiStart < 0 ? null : value.slice(apiStart);
}
...
function nearestNamedOwner(node: ts.Node, file: IndexedApiFile): ApiDeclarationRef {
  ...
  return { file, node, symbol: basename(file.path) };
}
...
function consumerFileIsExcluded(path: string): boolean {
  return path.includes('/__tests__/') ||
    ...
    path.includes('/features/agents/api/') ||
    path.endsWith('/api/openApiDocument.ts');
}
