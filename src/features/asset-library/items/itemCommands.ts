...
const { define: registerCommand, commands: itemCommands } = createCommandCollection();
export { itemCommands };
...
registerItemCommand({
  ...
  apply: (context) => {
    ...
    return commandSucceeded(`added item ${item.id} ('${item.symbol}')`);
  },
});
...
registerItemCommand({
  ...
  apply: (context, params) =>
    withItem(context, params, (itemId) => {
      ...
      return copy
        ? commandSucceeded(`duplicated item ${itemId} as ${copy.id}`)
        : commandFailed('unknown_item', `could not duplicate item ${itemId}`);
    }),
});
...
registerItemCommand({
  ...
  apply: (context, params) =>
    withItem(context, params, (itemId) => {
      ...
      return commandSucceeded(`removed item ${itemId}`);
    }),
});
...
function updateItem(context: CommandContext, params: CommandParams): CommandResult {
  return withItem(context, params, (itemId) => {
    ...
    return commandSucceeded(`item ${itemId} updated: ${listOf(Object.keys(patch.value))}`);
  });
}
...
function applyChoiceFields(
  ...
  if (render.ok) {
    if (!isItemRender(render.value)) {
      return { ok: false, failure: commandFailed('invalid_value', `'render' — ${renderHelp()}`) };
    }
  ...
  if (orientation.ok) {
    if (!isItemOrientation(orientation.value)) {
      return {
        ok: false,
        failure: commandFailed('invalid_value', `'orientation' — ${orientationHelp()}`),
      };
    }
...
export function tagsFrom(params: CommandParams, name = 'tags'): TagsRead {
  ...
  if (!Array.isArray(raw) || raw.some((tag) => typeof tag !== 'string')) {
    return {
      ok: false,
      failure: commandFailed('invalid_value', `'${name}' must be an array of tag strings`),
    };
  }
