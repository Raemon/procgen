...
export interface WorldSeedGenome {
  kitSeed: number;
...
export function rolledGenome(rng: RandomStream): WorldSeedGenome {
...
export function genomeFromJson(raw: unknown): WorldSeedGenome {
  const stored = (typeof raw === 'object' && raw !== null ? raw : {}) as Partial<WorldSeedGenome>;
...
export function genomeAsJson(genome: WorldSeedGenome): string {
