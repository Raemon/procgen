...
const WORLD_ROOT = 'src/features/asset-library/worlds';
const BARREL_PATH = `${WORLD_ROOT}/features/index.ts`;
...
function checkBarrelImportsEveryExtractorFile(check: CheckReporter): void {
  ...
  const extractorFiles = filesUnder(WORLD_ROOT, endingIn('Features.ts'));
  check(
...
function barrelSpecifierOf(path: string): string {
  const fromBarrel = relative(`${WORLD_ROOT}/features`, path).split(sep).join('/').replace(/\.ts$/, '');
