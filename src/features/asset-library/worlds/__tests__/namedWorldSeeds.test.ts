...
const SHIPPED_WORLDS = ['volcanic islands', 'infinite labyrinth', 'sunken labyrinth'];
...
function checkVolcanicIslandsRegenerates(check: CheckReporter): void {
  const islands = worldFromState(worldSeedStateNamed('volcanic islands'));
  const again = worldFromState(worldSeedStateNamed('volcanic islands'));
  check(
...
function aboveTheWaterline(
  ...
  const waterline = SEA_LEVEL * elevationHeightScaleOf(worldSeedStateNamed('volcanic islands'));
...
function deepestRavineCut(world: ReturnType<typeof worldFromState>): number {
  const scale = elevationHeightScaleOf(worldSeedStateNamed('volcanic islands'));
  let deepest = 0;
...
function checkInfiniteLabyrinthRegenerates(check: CheckReporter): void {
  const delve = worldFromState(worldSeedStateNamed('infinite labyrinth'));
  const again = worldFromState(worldSeedStateNamed('infinite labyrinth'));
  check(
...
function checkSavedWorldSeedsRoundTrip(check: CheckReporter): void {
  const saved = [{ name: 'kept', description: 'a saved world', state: earthlikeState() }];
  const restored = sanitizeWorldSeeds(JSON.parse(JSON.stringify(saved)));
  check(
