...
import { nodeIndexById, type NodeInstance } from '@/features/asset-library/worlds/pipeline/pipelineState';
import { isWireValid } from '@/features/asset-library/worlds/pipeline/wiringRules';
import {
  commandFailed,
  commandSucceeded,
  type CommandContext,
...
const { define: registerCommand, commands: nodeCommands } = createCommandCollection();
export { nodeCommands };
...
registerNodeCommand({
  ...
  params: {
    type: { kind: 'text', help: 'a node type id — see GET /api/v1/asset-library/node-types' },
    before_node_id: {
      kind: 'nodeId',
...
registerNodeCommand({
  ...
  apply: (context, params) =>
    withNode(context, params, (node) => {
      ...
      return copy
        ? commandSucceeded(`duplicated ${node.id} as ${copy.id}`)
        : commandFailed('unknown_node', `could not duplicate ${node.id}`);
    }),
});
...
registerNodeCommand({
  ...
  apply: (context, params) =>
    withNode(context, params, (node) => {
      ...
      return commandSucceeded(`removed ${node.id}; consumers rewired past it where possible`);
    }),
});
  ...
  registerNodeCommand({
    ...
    apply: (context, params) =>
      withNode(context, params, (node) => {
        ...
        return commandSucceeded(`${node.id} ${enabled ? 'enabled' : 'disabled'}`);
      }),
  });
}
...
registerNodeCommand({
  ...
  apply: (context, params) =>
    withNode(context, params, (node) => {
      ...
      return commandSucceeded(`renamed ${node.id} to '${label.value}'`);
    }),
});
...
registerNodeCommand({
  ...
  apply: (context, params) =>
    withNode(context, params, (node) => {
      ...
      return commandSucceeded(`comment set on ${node.id}`);
    }),
});
...
registerNodeCommand({
  ...
  apply: (context, params) =>
    withNode(context, params, (node) => {
      ...
      return commandSucceeded(folder === '' ? `${node.id} unfiled` : `${node.id} filed under '${folder}'`);
    }),
});
...
registerNodeCommand({
  ...
  params: {
    ...
    piece_id: {
      kind: 'int',
      help: 'pieces only: a piece id to stamp at each point — see GET /api/v1/asset-library/pieces',
      optional: true,
    },
    rotation: {
      kind: 'int',
    ...
    creature_id: {
      kind: 'int',
      help: 'creatures only: a creature id to spawn at each point — see GET /api/v1/asset-library/creatures',
      optional: true,
    },
    item_id: {
      kind: 'int',
      help: 'items only: an item id to float above each point — see GET /api/v1/asset-library/items',
      optional: true,
    },
  },
...
function withTypedNode(
  ...
  return withNode(context, params, (node) => {
    ...
    return def
      ? use(node, def)
      : commandFailed('unknown_node_type', `node ${node.id} has unregistered type ${node.type}`);
  });
}
...
function addNode(context: CommandContext, params: CommandParams): CommandResult {
  ...
  if (!node) {
    return commandFailed(
      'unknown_node_type',
      `no node type named '${type.value}' — see GET /api/v1/asset-library/node-types`,
    );
  }
  const placed = placeBeforeIfAsked(context, node, params);
  if (placed) return placed;
  return commandSucceeded(`added ${node.type} as ${node.id} (${describeWiring(node)})`);
}
...
function placeBeforeIfAsked(
  ...
  if (beforeIndex < 0) {
    context.store.removeNode(node.id);
    return commandFailed('unknown_node', `before_node_id '${before}' does not exist`);
  }
...
function moveNode(
  ...
  if (targetIndex < 0) {
    return commandFailed('unknown_node', `before_node_id '${String(before)}' does not exist`);
  }
  context.store.moveNodeToIndex(node.id, targetIndex);
  return commandSucceeded(`moved ${node.id}; wires to now-later sources were dropped if any`);
}
...
function setParam(
  ...
  return commandSucceeded(`${node.id}.${name.value} = ${JSON.stringify(accepted.value)}`);
}

type AcceptedParam = { ok: true; value: number | string } | { ok: false; failure: CommandResult };
...
function acceptKnobValue(
  ...
  if (spec.kind === 'toggle') {
    return value === 0 || value === 1
      ? { ok: true, value }
      : { ok: false, failure: commandFailed('invalid_value', `'${spec.label}' is a toggle: 0 or 1`) };
  }
  if (spec.kind === 'choice') {
...
function acceptScriptValue(
  ...
  if (typeof raw !== 'string') {
    return { ok: false, failure: commandFailed('invalid_value', `'${spec.label}' takes a string`) };
  }
  if (spec.kind === 'select' && !spec.options.includes(raw)) {
    return {
      ok: false,
      failure: commandFailed('invalid_value', `'${spec.label}' options: ${listOf(spec.options)}`),
    };
  }
...
function wireInput(
  ...
  if (source !== null && typeof source !== 'string') {
    return commandFailed('invalid_value', 'source_node_id must be a node id string or null');
  }
  if (source !== null) {
  ...
  return commandSucceeded(`${node.id}.${name.value} ← ${source ?? 'disconnected'}`);
}
...
function rejectInvalidWire(
  ...
  if (!context.store.nodeById(source)) {
    return commandFailed('unknown_node', `source_node_id '${source}' does not exist`);
  }
  const state = context.store.snapshot();
...
function setDisplay(
  ...
  return commandSucceeded(`${node.id} display = ${mode}`);
}
...
function rejectMissingBindingTarget(
  ...
  if (binding.mode === 'pieces' && binding.pieceId !== -1 && !context.pieces.byId(binding.pieceId)) {
    return commandFailed(
      'invalid_value',
      `piece_id must be -1 or one of: ${listOf(context.pieces.all().map((piece) => piece.id))} — see GET /api/v1/asset-library/pieces`,
    );
  }
  if (
    ...
    return commandFailed(
      'invalid_value',
      `creature_id must be -1 or one of: ${listOf(context.creatures.all().map((creature) => creature.id))} — see GET /api/v1/asset-library/creatures`,
    );
  }
  if (binding.mode === 'items' && binding.itemId !== -1 && !context.items.byId(binding.itemId)) {
    return commandFailed(
      'invalid_value',
      `item_id must be -1 or one of: ${listOf(context.items.all().map((item) => item.id))} — see GET /api/v1/asset-library/items`,
    );
  }
  return null;
}
