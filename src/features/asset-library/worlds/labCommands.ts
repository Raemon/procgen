...
const LAB_CONTROL = 'no button of its own — the world lab runs on the server, under /api/v1/asset-library/worlds';
...
registerLabCommand({
  ...
  changesWorld: false,
  description:
    'Grade the world you are standing in: a tourist walks it and reports one fun score plus a reading per metric band. The walk runs in the background — read_world_lab gives you the result and names the weakest readings, which is where to edit next.',
  params: {
  ...
  example: { action: 'grade_world', step_budget: 350 },
...
registerLabCommand({
  ...
  changesWorld: false,
  description:
    'Roll fresh worlds and grade every one of them, ranked by fun. This is the world generator and the grader in one pass: it neither touches nor replaces the world you are in. install_lab_worlds saves the winners into the library.',
  params: {
  ...
  example: { action: 'roll_worlds', count: 8 },
...
registerLabCommand({
  ...
  example: { action: 'train_worlds', generations: 20 },
  apply: (context, params) => startTraining(context, params),
});

registerLabCommand({
  action: 'read_world_lab',
  changesWorld: false,
  description:
  ...
  example: { action: 'read_world_lab', run_id: 'lab_1' },
...
registerLabCommand({
  ...
  params: { run_id: { kind: 'text', help: 'the run to stop — see read_world_lab' } },
  example: { action: 'stop_lab_run', run_id: 'lab_1' },
  apply: (context, params) => stopRun(context, params),
});

registerLabCommand({
  action: 'install_lab_worlds',
  changesWorld: true,
  description:
    "Save a run's best worlds into the library, each with the tiles, pieces and culture its palette needs, so run_world can run them by name.",
  params: {
...
function labOf(context: CommandContext): WorldLab | null {
  return context.lab;
}
...
function noLab(): CommandResult {
  return commandFailed(
    'no_world_lab',
    'this client has no world lab — the lab runs on the server, so call it from an agent or POST /api/v1/asset-library/worlds/grade, /roll or /train',
  );
}
...
function startGrade(context: CommandContext, params: CommandParams): CommandResult {
  ...
  return commandSucceeded(`${run.id} is walking this world; read_world_lab run_id=${run.id}`);
}
...
function startRoll(context: CommandContext, params: CommandParams): CommandResult {
  ...
  return commandSucceeded(
    `${run.id} is rolling and grading ${run.total} worlds from seed ${run.settings.seed}; read_world_lab run_id=${run.id}`,
  );
}
...
function startTraining(context: CommandContext, params: CommandParams): CommandResult {
  ...
  return commandSucceeded(
    `${run.id} is breeding ${run.settings.batch_size} worlds a generation for up to ${run.settings.generations} generations; read_world_lab run_id=${run.id}`,
  );
}
...
function installWorlds(context: CommandContext, params: CommandParams): CommandResult {
  ...
  return commandSucceeded(
    `saved ${installed.map((each) => `'${each.name}'`).join(', ')} into the library; run_world runs them by name`,
  );
}

function takenWorldNames(context: CommandContext): Set<string> {
  return new Set([
...
function unknownRun(lab: WorldLab, id: string): CommandResult {
