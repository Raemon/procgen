...
export function revealPacing(revealPerStep: readonly number[]): RevealPacing {
  return {
...
function droughtShareOfWalk(revealPerStep: readonly number[]): number {
  if (revealPerStep.length === 0) return 0;
  return longestRunOfNothingNew(revealPerStep) / revealPerStep.length;
}

function longestRunOfNothingNew(revealPerStep: readonly number[]): number {
  let longest = 0;
  let running = 0;
