...
import type { VoxelTool } from '../usePieceEditor';

export const PIECE_NAME_TIP: TooltipContent = {
  title: 'piece name',
  body: 'How the piece is listed in the display dropdown of a points node — where pieces get scattered into the world from.',
};

export const ADD_PIECE_TIP: TooltipContent = {
  title: 'add piece',
...
export const VOXEL_TOOL_TIPS: Readonly<Record<VoxelTool, TooltipContent>> = {
  ...
  face: {
    title: 'face',
    body: 'Paints the selected facing onto cells, turning panels, stairs and ramps in place. Facings turn with the piece when it rotates.',
  },
};

export const FACING_TIP: TooltipContent = {
  title: 'facing',
  body: 'Which way painted cells point: cycles north, east, south, west. The arrow on each cell shows the facing it already has.',
};

export const PIECE_ROLE_TIP: TooltipContent = {
  title: 'piece role',
  body: 'What part of a building this piece is. The assembler picks pieces by role — walls for walls, roof edges for eaves — so a mis-tagged piece lands in the wrong place.',
};
...
export const PIECE_EDIT_TIPS = {
  ...
  copy: { title: 'copy layer', body: 'Puts this layer on the clipboard shared by every piece.' },
  paste: {
    title: 'paste layer',
...
export function editPieceTip(open: boolean): TooltipContent {
  return {
...
export function duplicatePieceTip(piece: Piece): TooltipContent {
  return {
    title: `duplicate ${piece.name}`,
    body: 'Copies the stamp and its voxels into a new entry, leaving this one untouched.',
  };
}

export function deletePieceTip(piece: Piece): TooltipContent {
  return {
