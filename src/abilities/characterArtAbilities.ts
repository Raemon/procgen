...
import { clampFps } from '../creatures/character/sanitizeCharacterBillboard';
...
import { spriteFrom } from './itemAbilities';
...
function setFrame(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
  return withClip(context, params, (creatureId, billboard, rotation, animation) => {
    ...
    if (!sprite.value) return abilityFailed('invalid_value', "'sprite' must be sprite art, not null");
    ...
    return abilitySucceeded(`${rotation}/${animation} frame ${index.value} painted on creature ${creatureId}`);
  });
}

function removeFrame(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
  return withClip(context, params, (creatureId, billboard, rotation, animation) => {
    const frames = framesOf(billboard, rotation, animation);
    const index = readInt(params, 'frame');
    if (!index.ok) return index.failure;
    ...
    return abilitySucceeded(`${rotation}/${animation} frame ${index.value} dropped from creature ${creatureId}`);
  });
}

function setFps(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
  return withCreature(context, params, (creatureId) => {
    ...
    return abilitySucceeded(`creature ${creatureId} plays ${animation.value} at ${clamped} fps`);
  });
}
...
function rotationFrom(
  params: Record<string, unknown>,
): { ok: true; value: CharacterRotation } | { ok: false; failure: AbilityResult } {
  const read = readText(params, 'rotation');
  if (!read.ok) return read;
  if (!isCharacterRotation(read.value)) {
    return { ok: false, failure: abilityFailed('invalid_value', `'rotation' — ${rotationHelp()}`) };
  }
...
function animationFrom(
  params: Record<string, unknown>,
): { ok: true; value: CharacterAnimation } | { ok: false; failure: AbilityResult } {
  const read = readText(params, 'animation');
  if (!read.ok) return read;
  if (!isCharacterAnimation(read.value)) {
    return { ok: false, failure: abilityFailed('invalid_value', `'animation' — ${animationHelp()}`) };
  }
  return { ok: true, value: read.value };
}
