...
import { CHARACTER } from '../creatures/entityKinds';
import {
  blankInventory,
  clampSide,
  isInsideInventory,
  ...
  withSlotAt,
  type InventoryDef,
} from '../items/inventory/inventoryDef';
import {
  placementCovering,
  placementRefusal,
  prunedPlacements,
...
} from '../items/inventory/inventoryPlacement';
import {
  abilityFailed,
  abilitySucceeded,
  type AbilityContext,
...
import { withCreature } from './creatureAbilities';
import { spriteFrom, tagsFrom, withItem } from './itemAbilities';

const CREATURE_ID_HELP = 'id of an existing creature or character — see GET /api/v1/creatures';
...
registerInventoryAbility({
  ...
  humanControl: 'library panel, characters tab: the inventory width and height knobs',
  description:
...
registerInventoryAbility({
  ...
  humanControl: 'library panel, characters tab: remove inventory',
  ...
  apply: (context, params) =>
    withCreature(context, params, (creatureId) => {
      ...
      return abilitySucceeded(`creature ${creatureId} no longer has an inventory`);
    }),
});

registerInventoryAbility({
  action: 'update_inventory_slot',
  humanControl: 'library panel, characters tab: click a slot in the inventory grid',
  description:
...
registerInventoryAbility({
  ...
  humanControl: 'library panel, characters tab: the inventory backdrop art editor',
  description:
...
registerInventoryAbility({
  ...
  humanControl: 'library panel, characters tab: pick an item, then click a slot',
  description:
  ...
  params: {
    creature_id: { kind: 'int', help: CREATURE_ID_HELP },
    item_id: { kind: 'int', help: 'id of an existing item — see GET /api/v1/items' },
    slot_x: { kind: 'int', help: SLOT_X_HELP },
    slot_y: { kind: 'int', help: SLOT_Y_HELP },
  },
...
registerInventoryAbility({
  ...
  humanControl: 'library panel, characters tab: click a placed item in the inventory grid',
...
function setInventory(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
...
function updateSlot(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
  return withSlot(context, params, (creatureId, inventory, x, y) => {
    ...
    return abilitySucceeded(`slot ${x},${y} of creature ${creatureId} updated`);
  });
}

function setBackground(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
...
function placeItem(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
  return withSlot(context, params, (creatureId, inventory, x, y) =>
    withItem(context, params, (itemId) => {
      ...
      if (refusal) return abilityFailed('placement_refused', REFUSAL_HINTS[refusal]);
      saveInventory(context, creatureId, withItemPlaced(inventory, item, x, y));
      return abilitySucceeded(`placed item ${itemId} at ${x},${y} of creature ${creatureId}`);
    }),
  );
}

function removeItem(context: AbilityContext, params: Record<string, unknown>): AbilityResult {
  return withSlot(context, params, (creatureId, inventory, x, y) => {
    const covering = placementCovering(inventory, context.items, x, y);
    if (!covering) return abilityFailed('placement_refused', `no item covers slot ${x},${y}`);
    saveInventory(context, creatureId, withoutPlacementCovering(inventory, context.items, x, y));
    return abilitySucceeded(`removed item ${covering.itemId} from creature ${creatureId}`);
  });
}
...
function withInventory(
  ...
  use: (creatureId: number, inventory: InventoryDef) => AbilityResult,
): AbilityResult {
  return withCreature(context, params, (creatureId) => {
...
function withSlot(
  context: AbilityContext,
  params: Record<string, unknown>,
  use: (creatureId: number, inventory: InventoryDef, x: number, y: number) => AbilityResult,
): AbilityResult {
  return withInventory(context, params, (creatureId, inventory) => {
