...
Quest markers: a point tagged key:<id> is a key — stepping onto its cell adds
it to your keys (see keys_held in observations, and the pickup summary on the
act response). A point tagged door:<id> keeps its cell impassable to you until
you hold the matching key:<id>; stepping into it fails with locked_door. Keys
are per-agent and never run out.
