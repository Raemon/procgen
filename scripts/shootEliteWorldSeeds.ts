...
import { funOf, walkSeedOf, type ScoredWorldSeed } from '@/features/asset-library/worlds/selfPlay/scoreGenome';
import { runTraining } from '@/features/asset-library/worlds/selfPlay/trainingLoop';
import { genomeAsJson } from '@/features/asset-library/worlds/selfPlay/worldSeedGenome';
...
function showcaseSlateOf(ranked: readonly ScoredWorldSeed[]): ScoredWorldSeed[] {
...
async function shootWorld(elite: ScoredWorldSeed, index: number): Promise<WorldShotRecord> {
...
function shotPointsFor(elite: ScoredWorldSeed): ShotPoint[] {
  const world = worldOfGenome(elite.genome);
