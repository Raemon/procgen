...
export function checkOnlyTheAbilityLayerCanMutate(
  ...
  const offenders = sourceFiles(SOURCE_ROOT).filter(importsAMutableLibrary);
  ...
  check(
    ...
    sourceFiles('src/ui').every((path) => !readFileSync(path, 'utf8').includes('runtime.store.set')),
  );
}
