...
const NORTH_VIEW = { yaw: 0, worldPerPixel: 0.1, pitchRadians: Math.PI / 2 };

export function checkCameraMovementAndZoom(check: CheckReporter): void {
  checkForwardIsWhicheverWayTheCameraIsFacing(check);
  checkTheWheelZoomsSmoothlyAndStopsAtItsLimits(check);
  checkPanningAccumulatesUntilTheCameraIsRecentered(check);
  checkDraggingMovesTheCameraSoTheWorldFollowsTheCursor(check);
  checkAShallowerPitchCoversMoreGroundPerDragPixel(check);
  checkStreamingReachGrowsWithZoomButStaysCapped(check);
}

function checkForwardIsWhicheverWayTheCameraIsFacing(check: CheckReporter): void {
...
function checkTheWheelZoomsSmoothlyAndStopsAtItsLimits(check: CheckReporter): void {
...
function checkPanningAccumulatesUntilTheCameraIsRecentered(check: CheckReporter): void {
...
function checkDraggingMovesTheCameraSoTheWorldFollowsTheCursor(check: CheckReporter): void {
  const draggedRight = worldPanForDrag({ dxPixels: 10, dyPixels: 0 }, NORTH_VIEW);
  check(
  ...
  const draggedDown = worldPanForDrag({ dxPixels: 0, dyPixels: 10 }, NORTH_VIEW);
  check(
...
function checkAShallowerPitchCoversMoreGroundPerDragPixel(check: CheckReporter): void {
  const shallowPitch = worldPanForDrag({ dxPixels: 0, dyPixels: 10 }, { ...NORTH_VIEW, pitchRadians: Math.PI / 6 });
  check('a shallower pitch covers more ground per vertical drag pixel', Math.abs(shallowPitch.dy) > 1);
}

function checkStreamingReachGrowsWithZoomButStaysCapped(check: CheckReporter): void {
