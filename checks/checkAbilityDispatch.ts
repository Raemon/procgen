...
type AbilityWorld = ReturnType<typeof abilityWorld>;

type Act = (
  mode: 'god' | 'character',
  action: string,
  params?: Record<string, unknown>,
) => ReturnType<typeof performAbility>;

export function checkAbilityDispatch(check: CheckReporter): void {
  const abilities = abilityWorld();
  const act: Act = (mode, action, params = {}) =>
    performAbility(abilities.context, mode, action, params);

  checkAnAbilityAskedOfTheWrongModeIsRefusedWithGuidance(check, act);
  checkMovingAndTurningGoThroughTheRegistryTheApiUses(check, act, abilities);
  checkACharacterCanWidenItsOwnSightWithinTheRangeTheDocsPromise(check, act, abilities);
  const noiseId = checkTheGraphOnlyGrowsThroughValidatedNodeEdits(check, act, abilities);
  const thresholdId = checkWiringOnlyEverPointsBackThroughTheGraph(check, act, abilities, noiseId);
  checkADisplayModeMustSuitTheOutputKindItRenders(check, act, abilities, noiseId);
  checkSetSeedReseedsThePipeline(check, act, abilities);
  checkADisplayRefusesAnAssetIdNothingOwns(check, act, abilities);
  checkRemoveNodeTakesTheNodeOutOfTheGraph(check, act, abilities, thresholdId);
  checkTilesCanBeAuthoredThroughAbilities(check, act, abilities);
  checkPiecesCanBeAuthoredThroughAbilities(check, act, abilities);
  checkCreaturesCanBeAuthoredThroughAbilities(check, act, abilities);
  checkSavedWorkCanBeNamedAndFetchedBackByName(check, act, abilities);
  checkARollCanBeSeededAndUndone(check, act, abilities);
  checkCaptureRegionLiftsTheWorldIntoAPiece(check, act, abilities);
  checkTheRegistryAndTheApiAgreeOnWhatEachModeOwns(check);
}

function checkAnAbilityAskedOfTheWrongModeIsRefusedWithGuidance(
  check: CheckReporter,
  act: Act,
): void {
...
function checkMovingAndTurningGoThroughTheRegistryTheApiUses(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkACharacterCanWidenItsOwnSightWithinTheRangeTheDocsPromise(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkTheGraphOnlyGrowsThroughValidatedNodeEdits(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): string {
...
function checkWiringOnlyEverPointsBackThroughTheGraph(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
  noiseId: string,
...
function checkADisplayModeMustSuitTheOutputKindItRenders(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
  noiseId: string,
...
function checkSetSeedReseedsThePipeline(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkADisplayRefusesAnAssetIdNothingOwns(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkRemoveNodeTakesTheNodeOutOfTheGraph(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
  thresholdId: string,
...
function checkTilesCanBeAuthoredThroughAbilities(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkPiecesCanBeAuthoredThroughAbilities(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkCreaturesCanBeAuthoredThroughAbilities(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkSavedWorkCanBeNamedAndFetchedBackByName(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkARollCanBeSeededAndUndone(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkCaptureRegionLiftsTheWorldIntoAPiece(
  check: CheckReporter,
  act: Act,
  abilities: AbilityWorld,
): void {
...
function checkTheRegistryAndTheApiAgreeOnWhatEachModeOwns(check: CheckReporter): void {
