...
import { exampleWorlds } from '../procgen/presets/exampleWorlds';
...
function worldEditor() {
  ...
  const contextAround = (edited: PipelineStore): AbilityContext =>
    ({
      store: edited,
  ...
  const performOn = (edited: PipelineStore, action: string, params?: Record<string, unknown>) =>
    performAbility(contextAround(edited), 'god', action, params);
