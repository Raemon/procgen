...
type FaceArt = ReturnType<typeof blankCubeFaceArt>;

interface AnimatedArt {
  stillArt: FaceArt;
  paintedSecondFrame: FaceArt;
}

export function checkFaceArtEditingOps(check: CheckReporter): void {
  const art = checkFaceArtIsValidatedAndClonedWithoutSharingItsPixels(check);
  checkLegacyThreeFacedArtUpgradesToSixFaces(check);
  checkResizingRescalesThePaintedPixelsAndSurvivesTheReturnTrip(check, art);
  checkShiftingWrapsPixelsAroundTheEdge(check);
  checkFloodFillStopsWhereAnotherColourStartsIt(check);
  checkMirroredPaintingHitsEveryMirroredIndex(check);
  checkSidesStayLinkedUntilOneIsPaintedApart(check);
  const animated = checkALaterFrameOnlyOverridesTheFacesItRepaints(check);
  checkAnimatedArtSurvivesACloneAndARoundTripThroughJson(check, animated);
  checkFramesCanBeDroppedButNeverTheLastOne(check, animated);
  const reliefArt = checkAReliefLayerRidesAlongsideTheColoursItShades(check);
  checkResizingCarriesTheReliefLayerAndEveryFrameWithIt(check, reliefArt, animated);
  checkADifferencePaintedOnALaterFrameStillUnmatchesTheSides(check);
}

function checkFaceArtIsValidatedAndClonedWithoutSharingItsPixels(check: CheckReporter): FaceArt {
...
function checkLegacyThreeFacedArtUpgradesToSixFaces(check: CheckReporter): void {
...
function checkResizingRescalesThePaintedPixelsAndSurvivesTheReturnTrip(
  check: CheckReporter,
  art: FaceArt,
): void {
...
function checkShiftingWrapsPixelsAroundTheEdge(check: CheckReporter): void {
...
function checkFloodFillStopsWhereAnotherColourStartsIt(check: CheckReporter): void {
...
function checkMirroredPaintingHitsEveryMirroredIndex(check: CheckReporter): void {
  check(
...
function checkSidesStayLinkedUntilOneIsPaintedApart(check: CheckReporter): void {
...
function checkALaterFrameOnlyOverridesTheFacesItRepaints(check: CheckReporter): AnimatedArt {
  ...
  return { stillArt, paintedSecondFrame };
}

function checkAnimatedArtSurvivesACloneAndARoundTripThroughJson(
  check: CheckReporter,
  { paintedSecondFrame }: AnimatedArt,
): void {
  check(
...
function checkFramesCanBeDroppedButNeverTheLastOne(
  check: CheckReporter,
  { stillArt, paintedSecondFrame }: AnimatedArt,
): void {
  check(
...
function checkAReliefLayerRidesAlongsideTheColoursItShades(check: CheckReporter): FaceArt {
...
function checkResizingCarriesTheReliefLayerAndEveryFrameWithIt(
  check: CheckReporter,
  reliefArt: FaceArt,
  { paintedSecondFrame }: AnimatedArt,
): void {
...
function checkADifferencePaintedOnALaterFrameStillUnmatchesTheSides(check: CheckReporter): void {
  check(
